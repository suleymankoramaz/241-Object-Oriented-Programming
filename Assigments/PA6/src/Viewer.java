...
public class Viewer extends Observer{
	...
	//Constructor
    public Viewer(){play_list = new ArrayList<data>();}
    ...
    //THIS FUNCTION JUST USING NP BECAUSE THIS CLASS IS VIEWER , SO IT'S USING JUST NON_PLAYABLE OBJECTS
    public void set_list(ArrayList<data> p,ArrayList<data> np)
    {
    	...
    	for(int i=0;i<np.size();i++){
        	this.play_list.add(np.get(i));
        	if(current_data == np.get(i))
            	control++;
    	}
    ...
    public  void show_list()
    {
        //Player can't receive list
      	if(this.subject == null)
            System.out.println("Viewer can't receive list .");
        //List is empty
        else if(play_list.size()==0)
            System.out.println("List is empty.");
        //Otherwise print play_list
        else{
            System.out.println("Playlist of this viewer:");
    ...
    public  void next(String type)
    {
        ...
        for(int i=0;i<play_list.size();i++){
            ...
            if(current_data.get_name() == play_list.get(i).get_name())
                ...
                for(int j=i+1;j<play_list.size();j++){
                    ...
                    //If current_data is visual find next visual data
                    if(play_list.get(j) instanceof visual && type == "image"){
                        new_index = j;
                        break;
                    }
                    //If current_data is not visual find next visual data
                    else if(play_list.get(j) instanceof non_visual && type == "text"){
                        new_index = j;
                        break;
                    }
                }           
        ...
        if(new_index == -1){
            for(int i=0;i<play_list.size();i++){
                if(play_list.get(i) instanceof visual && type == "image"){
                    current_data = play_list.get(i);
                    break;
                }
                else if(play_list.get(i) instanceof non_visual && type == "text"){
                    current_data = play_list.get(i);
                    break;
                }
            }
